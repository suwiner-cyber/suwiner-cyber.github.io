...
public class ReaderActivity extends Activity {
    ...
    private final StablePageAnimator pageAnimator=new StablePageAnimator();
    private CatalogStore.Data catalog;
    private int chapterIndex=0,pageIndex=0;
    private volatile int loadToken=0;
    private boolean immersive=false,pageSound=true;
    ...
    @Override protected void onPause(){pageAnimator.cancel(body);saveProgress();super.onPause();}
    @Override protected void onDestroy(){pageAnimator.cancel(body);super.onDestroy();}
    ...
    private void setupGestures(){gestures=new GestureDetector(this,new GestureDetector.SimpleOnGestureListener(){
        ...
        @Override public boolean onFling(MotionEvent e1,MotionEvent e2,float vx,float vy){if(e1==null||e2==null)return false;float dx=e2.getX()-e1.getX(),dy=e2.getY()-e1.getY();if(Math.abs(dx)<dp(52)||Math.abs(dx)<Math.abs(dy)*1.22f)return false;if(dx<0)nextPage();else previousPage();return true;}
    });}
    ...
    private void nextPage(){
        if(pageAnimator.isRunning()||pages.isEmpty())return;
        if(pageIndex<pages.size()-1){playPageSound();pageAnimator.turn(body,1,()->{pageIndex++;body.setText(pages.get(pageIndex));updateProgress();saveProgress();},null);return;}
        if(catalog!=null&&chapterIndex<catalog.chapters.size()-1){pageAnimator.cancel(body);playPageSound();loadChapter(chapterIndex+1,0);}else Toast.makeText(this,"已经是最后一页",Toast.LENGTH_SHORT).show();
    }
    private void previousPage(){
        if(pageAnimator.isRunning()||pages.isEmpty())return;
        if(pageIndex>0){playPageSound();pageAnimator.turn(body,-1,()->{pageIndex--;body.setText(pages.get(pageIndex));updateProgress();saveProgress();},null);return;}
        if(catalog!=null&&chapterIndex>0){pageAnimator.cancel(body);playPageSound();loadChapter(chapterIndex-1,Integer.MAX_VALUE);}else Toast.makeText(this,"已经是第一页",Toast.LENGTH_SHORT).show();
    }
    private void playPageSound(){if(pageSound)PageTurnSound.play(this);}

    private void setImmersive(boolean on,boolean persist){
        pageAnimator.cancel(body);immersive=on;if(top!=null)top.setVisibility(on?View.GONE:View.VISIBLE);if(chapterTitle!=null)chapterTitle.setVisibility(on?View.GONE:View.VISIBLE);if(bottom!=null)bottom.setVisibility(on?View.GONE:View.VISIBLE);if(progress!=null)progress.setVisibility(on?View.GONE:View.VISIBLE);if(body!=null)body.setPadding(dp(26),on?dp(28):dp(16),dp(26),on?dp(30):dp(20));int ui=on?(View.SYSTEM_UI_FLAG_FULLSCREEN|View.SYSTEM_UI_FLAG_HIDE_NAVIGATION|View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY|View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN|View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION|View.SYSTEM_UI_FLAG_LAYOUT_STABLE):(bestTextColor(bg)==Color.rgb(48,44,38)?View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR:0);getWindow().getDecorView().setSystemUiVisibility(ui);if(persist)getSharedPreferences("reader_settings",MODE_PRIVATE).edit().putBoolean("immersive",immersive).apply();body.postDelayed(()->paginateAndShow(Math.min(pageIndex,Math.max(0,pages.size()-1))),120);
    }

    private void loadChapter(int idx,int wantedPage){
        pageAnimator.cancel(body);
    ...
    private void paginateAndShow(int wantedPage){
        if(body.getWidth()<=0||body.getHeight()<=0){body.postDelayed(()->paginateAndShow(wantedPage),80);return;}pageAnimator.cancel(body);pages.clear();String text=currentChapterText==null?"":currentChapterText;if(text.length()==0){pages.add("");pageIndex=0;renderPage();return;}int width=body.getWidth()-body.getPaddingLeft()-body.getPaddingRight(),height=body.getHeight()-body.getPaddingTop()-body.getPaddingBottom();if(width<dp(80)||height<dp(100)){body.postDelayed(()->paginateAndShow(wantedPage),100);return;}
    ...
    private void renderPage(){if(pages.isEmpty())return;pageAnimator.cancel(body);pageIndex=Math.max(0,Math.min(pageIndex,pages.size()-1));body.setText(pages.get(pageIndex));updateProgress();}
    ...
    private void setBackgroundTheme(int color){pageAnimator.cancel(body);bg=color;fg=bestTextColor(color);applyTheme();savePrefs();}
    ...
    private void applyAndSave(){pageAnimator.cancel(body);body.setTextSize(fontSize);body.setLineSpacing(dp(lineExtra),1f);savePrefs();body.post(()->paginateAndShow(Math.min(pageIndex,Math.max(0,pages.size()-1))));}
