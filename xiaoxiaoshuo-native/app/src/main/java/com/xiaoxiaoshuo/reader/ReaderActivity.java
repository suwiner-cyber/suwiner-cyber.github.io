...
public class ReaderActivity extends Activity {
    ...
    private int chapterIndex=0, pageIndex=0, requestedPageAfterLoad=0;
    ...
    @Override public void onCreate(Bundle b){
        ...
        bookTitle=n(getIntent().getStringExtra("title"));
        ...
        if(catalog!=null&&!catalog.chapters.isEmpty()){
            ...
            int initial=Math.max(0,Math.min(chapterIndex,catalog.chapters.size()-1));
            int savedPage=getSharedPreferences("reading_progress",MODE_PRIVATE).getInt(pageKey(),0);
            loadChapter(initial,savedPage);
        }else{
            String raw=getIntent().getStringExtra("body");
            setChapterText(AiTypesetter.formatNovel(raw==null?"正文加载失败。":raw),0);
        }
    ...
    private void build(){
        ...
        body=new TextView(this);body.setPadding(dp(26),dp(16),dp(26),dp(20));body.setTextSize(fontSize);body.setLineSpacing(dp(lineExtra),1f);body.setTextIsSelectable(true);body.setGravity(Gravity.TOP|Gravity.LEFT);applyFont();
        root.addView(body,new LinearLayout.LayoutParams(-1,0,1));

        bottom=new LinearLayout(this);bottom.setGravity(Gravity.CENTER);bottom.setPadding(dp(10),dp(5),dp(10),dp(6));
        prev=nav("‹ 上一页");prev.setOnClickListener(v->previousPage());
        toc=nav("目录");toc.setOnClickListener(v->showCatalog());
        next=nav("下一页 ›");next.setOnClickListener(v->nextPage());
        bottom.addView(prev,new LinearLayout.LayoutParams(0,dp(44),1));
        bottom.addView(toc,new LinearLayout.LayoutParams(0,dp(44),1));
        bottom.addView(next,new LinearLayout.LayoutParams(0,dp(44),1));root.addView(bottom);
        progress=label("",11);progress.setGravity(Gravity.CENTER);root.addView(progress,new LinearLayout.LayoutParams(-1,dp(20)));
        setContentView(root);applyTheme();
    }

    private void setupGestures(){
        gestures=new GestureDetector(this,new GestureDetector.SimpleOnGestureListener(){
            @Override public boolean onDown(MotionEvent e){return true;}
            @Override public boolean onDoubleTap(MotionEvent e){if(immersive){setImmersive(false,true);return true;}return false;}
            @Override public boolean onFling(MotionEvent e1,MotionEvent e2,float vx,float vy){
                if(e1==null||e2==null)return false;
                float dx=e2.getX()-e1.getX(),dy=e2.getY()-e1.getY();
                if(Math.abs(dx)<dp(60)||Math.abs(dx)<Math.abs(dy)*1.3f)return false;
    ...
    private void nextPage(){
        ...
        if(pageIndex<pages.size()-1){pageIndex++;renderPage(1);saveProgress();return;}
        if(catalog!=null&&chapterIndex<catalog.chapters.size()-1)loadChapter(chapterIndex+1,0);
        else Toast.makeText(this,"已经是最后一页",Toast.LENGTH_SHORT).show();
    }
    private void previousPage(){
        if(pages.isEmpty())return;
        if(pageIndex>0){pageIndex--;renderPage(-1);saveProgress();return;}
        if(catalog!=null&&chapterIndex>0)loadChapter(chapterIndex-1,Integer.MAX_VALUE);
        else Toast.makeText(this,"已经是第一页",Toast.LENGTH_SHORT).show();
    }

    private void setImmersive(boolean on,boolean persist){
        immersive=on;
        if(top!=null)top.setVisibility(on?View.GONE:View.VISIBLE);
        if(chapterTitle!=null)chapterTitle.setVisibility(on?View.GONE:View.VISIBLE);
        if(bottom!=null)bottom.setVisibility(on?View.GONE:View.VISIBLE);
        if(progress!=null)progress.setVisibility(on?View.GONE:View.VISIBLE);
        if(body!=null)body.setPadding(dp(26),on?dp(28):dp(16),dp(26),on?dp(30):dp(20));
        int ui=on?(View.SYSTEM_UI_FLAG_FULLSCREEN|View.SYSTEM_UI_FLAG_HIDE_NAVIGATION|View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY|View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN|View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION|View.SYSTEM_UI_FLAG_LAYOUT_STABLE):(bestTextColor(bg)==Color.rgb(48,44,38)?View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR:0);
        getWindow().getDecorView().setSystemUiVisibility(ui);
        if(persist)getSharedPreferences("reader_settings",MODE_PRIVATE).edit().putBoolean("immersive",immersive).apply();
    ...
    private void loadChapter(int idx,int wantedPage){
        ...
        chapterIndex=idx;requestedPageAfterLoad=wantedPage;
        LegacyDexBridge.Chapter ch=catalog.chapters.get(idx);
        chapterTitle.setText(ch.name);pages.clear();body.setText("正在加载正文…");updateProgress();
        int token=++loadToken;final int wantedIndex=idx;final String wantedName=ch.name,initialUrl=ch.url,bt=title.getText().toString();
        String cached=OfflineBookCache.getChapter(this,bt,idx,initialUrl);
        if(cached.trim().length()>0){setChapterText(AiTypesetter.formatNovel(cached),wantedPage);return;}
        new Thread(()->{
            String text="",switched="";CatalogStore.Data newCatalog=null;String newSource=sourceJson;int newIndex=wantedIndex;String cacheUrl=initialUrl;
            try{text=LegacyDexBridge.get(this).loadChapter(sourceJson,wantedName,initialUrl);}catch(Throwable ignored){}
            if(text.trim().length()==0){Alternate alt=findAlternate(bt,wantedName,wantedIndex);if(alt!=null&&alt.text.trim().length()>0){text=alt.text;switched=alt.sourceName;newCatalog=alt.catalog;newSource=alt.sourceJson;newIndex=alt.index;cacheUrl=alt.catalog.chapters.get(alt.index).url;}}
            if(text.trim().length()>0)OfflineBookCache.putChapter(this,bt,newIndex,cacheUrl,text);
            ...
            runOnUiThread(()->{
                ...
                if(finalCatalog!=null){
                    catalog=finalCatalog;sourceJson=finalSource;chapterIndex=finalIndex;catalogKey=CatalogStore.save(this,bt,sourceJson,catalog.chapters);
                    LegacyDexBridge.Chapter real=catalog.chapters.get(chapterIndex);chapterTitle.setText(real.name);
                    if(sourceName.length()>0)Toast.makeText(this,"已自动切换书源："+sourceName,Toast.LENGTH_SHORT).show();
                }
                String formatted=ready.trim().length()==0?"本章正文加载失败，已自动尝试全部优选书源，暂未找到可读正文。":AiTypesetter.formatNovel(ready);
                setChapterText(formatted,wantedPage);
            });
        }).start();
    }

    private void setChapterText(String text,int wantedPage){
        currentChapterText=text==null?"":text;
        requestedPageAfterLoad=wantedPage;
        body.post(()->paginateAndShow(wantedPage));
    }

    private void paginateAndShow(int wantedPage){
        if(body.getWidth()<=0||body.getHeight()<=0){body.postDelayed(()->paginateAndShow(wantedPage),80);return;}
        pages.clear();
        String text=currentChapterText==null?"":currentChapterText;
        if(text.length()==0){pages.add("");pageIndex=0;renderPage(0);return;}
        int width=body.getWidth()-body.getPaddingLeft()-body.getPaddingRight();
        int height=body.getHeight()-body.getPaddingTop()-body.getPaddingBottom();
        if(width<dp(80)||height<dp(100)){body.postDelayed(()->paginateAndShow(wantedPage),100);return;}
        try{
            TextPaint paint=new TextPaint(Paint.ANTI_ALIAS_FLAG);paint.setTextSize(body.getTextSize());paint.setTypeface(body.getTypeface());paint.setColor(fg);
            StaticLayout layout=StaticLayout.Builder.obtain(text,0,text.length(),paint,width).setAlignment(Layout.Alignment.ALIGN_NORMAL).setLineSpacing(dp(lineExtra),1f).setIncludePad(false).build();
            int startLine=0,totalLines=layout.getLineCount();
            while(startLine<totalLines){
                int pageTop=layout.getLineTop(startLine);int endLine=startLine;
                while(endLine+1<totalLines&&layout.getLineBottom(endLine+1)-pageTop<=height)endLine++;
                int start=layout.getLineStart(startLine);int end=layout.getLineEnd(endLine);
                if(end<=start){end=Math.min(text.length(),start+1);}
                pages.add(text.substring(start,end));
                startLine=endLine+1;
            }
        }catch(Throwable e){
            int approx=Math.max(260,(width/dp(20))*Math.max(8,height/dp(34)));
            for(int p=0;p<text.length();p+=approx)pages.add(text.substring(p,Math.min(text.length(),p+approx)));
        }
        if(pages.isEmpty())pages.add(text);
        pageIndex=wantedPage==Integer.MAX_VALUE?pages.size()-1:Math.max(0,Math.min(wantedPage,pages.size()-1));
        renderPage(0);saveProgress();
    }
    ...
    private void renderPage(int direction){
        ...
        body.animate().cancel();body.setText(page);
        if(direction!=0){body.setTranslationX(direction>0?dp(35):-dp(35));body.setAlpha(.72f);body.animate().translationX(0).alpha(1f).setDuration(150).start();}
        else{body.setTranslationX(0);body.setAlpha(1f);}
        updateProgress();
    }

    private void updateProgress(){
        int chTotal=catalog==null?0:catalog.chapters.size();
        int pgTotal=Math.max(1,pages.size());
        progress.setText((chapterIndex+1)+" / "+Math.max(1,chTotal)+"  ·  第 "+(pageIndex+1)+" / "+pgTotal+" 页");
    }
        ...
        for(LegacySourceStore.SourceInfo s:st.selected){
            ...
            try{
                ...
                if(text!=null&&text.trim().length()>0){
                    LegacySourceStore.recordHealth(this,s.url,true,System.currentTimeMillis()-start);
                    CatalogStore.Data d=new CatalogStore.Data();d.title=bookTitle;d.sourceJson=best.sourceJson;d.chapters.addAll(cs);OfflineBookCache.saveBook(this,best,cs);
                    Alternate a=new Alternate();a.text=text;a.sourceJson=best.sourceJson;a.sourceName=best.sourceName.length()>0?best.sourceName:s.name;a.catalog=d;a.index=target;return a;
                }
    ...
    private void saveProgress(){
        getSharedPreferences("reading_progress",MODE_PRIVATE).edit().putInt("idx_"+Integer.toHexString(title.getText().toString().hashCode()),chapterIndex).putInt(pageKey(),pageIndex).apply();
    }
    ...
    private void showCatalog(){
        ...
        for(int i=0;i<catalog.chapters.size();i++){final int q=i;TextView r=label((i+1)+"  "+catalog.chapters.get(i).name,14);r.setPadding(dp(10),dp(12),dp(10),dp(12));if(i==chapterIndex){r.setTextColor(navColor());r.setTypeface(Typeface.DEFAULT,Typeface.BOLD);}r.setOnClickListener(v->{d.dismiss();loadChapter(q,0);});list.addView(r);}
    ...
    private void showSettings(){
        final Dialog d=new Dialog(this);LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(dp(22),dp(18),dp(22),dp(20));GradientDrawable pbg=new GradientDrawable();pbg.setColor(Color.rgb(255,253,249));pbg.setCornerRadius(dp(24));panel.setBackground(pbg);TextView h=dark("阅读设置",22,true);panel.addView(h);
        LinearLayout im=row();TextView imText=dark("沉浸式阅读\n隐藏导航 · 左右滑页 · 双击退出",14,false);im.addView(imText,new LinearLayout.LayoutParams(0,dp(58),1));Switch sw=new Switch(this);sw.setChecked(immersive);im.addView(sw,new LinearLayout.LayoutParams(dp(64),dp(58)));panel.addView(im);sw.setOnCheckedChangeListener((v,on)->{setImmersive(on,true);if(on)d.dismiss();});
        ...
        TextView close=dark("完成",16,true);close.setGravity(Gravity.CENTER);close.setTextColor(Color.rgb(49,88,71));close.setPadding(0,dp(16),0,dp(6));close.setOnClickListener(v->d.dismiss());panel.addView(close);
    ...
    private void showCustomColorDialog(){
        ...
        new AlertDialog.Builder(this).setTitle("自定义阅读背景").setView(e).setNegativeButton("取消",null).setPositiveButton("应用",(d,w)->{try{String raw=e.getText().toString().trim();if(!raw.startsWith("#"))raw="#"+raw;setBackgroundTheme(Color.parseColor(raw));}catch(Throwable ex){Toast.makeText(this,"颜色格式不正确，例如 #F4EBD8",Toast.LENGTH_LONG).show();}}).show();
    }
    ...
    private void applyTheme(){
        if(root!=null)root.setBackgroundColor(bg);if(top!=null)top.setBackgroundColor(bg);if(bottom!=null)bottom.setBackgroundColor(bg);
        if(body!=null){body.setBackgroundColor(bg);body.setTextColor(fg);}if(title!=null)title.setTextColor(fg);if(back!=null)back.setTextColor(fg);if(catalogBtn!=null)catalogBtn.setTextColor(fg);if(setup!=null)setup.setTextColor(fg);
        if(chapterTitle!=null){chapterTitle.setBackgroundColor(bg);chapterTitle.setTextColor(mutedColor());}if(progress!=null){progress.setBackgroundColor(bg);progress.setTextColor(mutedColor());}
        if(prev!=null){prev.setBackgroundColor(bg);prev.setTextColor(navColor());}if(toc!=null){toc.setBackgroundColor(bg);toc.setTextColor(navColor());}if(next!=null){next.setBackgroundColor(bg);next.setTextColor(navColor());}
        getWindow().setStatusBarColor(bg);getWindow().setNavigationBarColor(bg);if(!immersive)getWindow().getDecorView().setSystemUiVisibility(bestTextColor(bg)==Color.rgb(48,44,38)?View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR:0);
    }
    ...
    private void savePrefs(){getSharedPreferences("reader_settings",MODE_PRIVATE).edit().putFloat("size",fontSize).putFloat("line",lineExtra).putInt("bg",bg).putInt("fg",fg).putString("font",fontKey).putBoolean("immersive",immersive).apply();}
    private void loadPrefs(){android.content.SharedPreferences p=getSharedPreferences("reader_settings",MODE_PRIVATE);fontSize=p.getFloat("size",20);lineExtra=p.getFloat("line",9);bg=p.getInt("bg",Color.rgb(248,244,232));fg=p.getInt("fg",Color.rgb(48,44,38));fontKey=p.getString("font","wenkai");immersive=p.getBoolean("immersive",false);}
